...
plugin.ogcapi.DataProvider.prototype.toChildNode = function (collection) {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        if ((link['type'] === 'application/json') && (link.hasOwnProperty('href') && (link['rel'] === 'tiles'))) {
            ...
            request.getPromise().
                then(this.onTileCollectionLoad.bind(this, id, extent, styleid), this.onError, this).
                thenCatch(this.onError, this);
 ...
 * @protected
 */
plugin.ogcapi.DataProvider.prototype.onTileCollectionLoad = function (id, extent, styleid, response) {
    try {
        var json = JSON.parse(response);
    } catch (e) {
